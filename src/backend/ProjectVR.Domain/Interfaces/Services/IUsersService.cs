...
namespace ProjectVR.Domain.Interfaces.Services
{
    public interface IUsersService
    {
        ...
        /// Если <paramref name="signedUserGuid"/> не null, то пользователь с таким Guid и его друзья, входящие заявки и исходящие заявки не попадают в поиск
        /// </returns>
        Task<UserSummary[]> FindUsersByGameOrVrSet(string query, int offset, int limit, Guid? signedUserGuid = null);

        /// <summary>
        /// Список пользователей, у которых есть игра либо vr гарнитура содержащая соответственно <paramref name="game"/> и <paramref name="vrset"/> в названии
        /// </summary>
