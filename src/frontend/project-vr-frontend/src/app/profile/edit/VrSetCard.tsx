...
import HoverIcon, { HoverIconProps } from "@/components/icons/HoverIcon";
...
export default function VrSetCard({
  ...
  const favoriteIconProps: HoverIconProps = {
    svgProps: {
      Icon: FavoriteIcon,
      props: {
        className: `${
  ...
  const deleteIconProps: HoverIconProps = {
    svgProps: {
      Icon: DeleteIcon,
      props: {
    ...
    <Card key={userVrSet.vrSetId} radius="none" className="mt-5 bg-transparent">
      <div className="relative">
        <div className="absolute flex right-0">
          <HoverIcon {...favoriteIconProps} />
          <HoverIcon {...deleteIconProps} />
        </div>
